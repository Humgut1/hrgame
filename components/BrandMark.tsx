...
export default function BrandMark({ product = 'grow', eyes = true, size, className, title }: {
  product?: Product
  eyes?: boolean
  size?: number
  className?: string
...
/** 로고 + 'talentcore' 글자. 글꼴(Unbounded 700)은 app/layout.tsx 가 --font-unbounded 로 불러온다. */
export function BrandLockup({ product = 'grow', size = 22, className }: { product?: Product; size?: number; className?: string }) {
  return (
    <span className={'inline-flex items-center gap-2 text-ink' + (className ? ' ' + className : '')}>
      <BrandMark product={product} size={size} />
      <span style={{ fontFamily: 'var(--font-unbounded), sans-serif', fontWeight: 700, letterSpacing: '-.03em', fontSize: size * 0.6 }}>
        talentcore
      </span>
    </span>
  )
